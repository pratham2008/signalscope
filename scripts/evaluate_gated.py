"""Evaluate the gated SignalScope detector on a manifest split."""
...
class GatedInferenceDataset(Dataset):
    """Return image, label, generator, and image path."""
    ...
    def __getitem__(self, index):
        ...
        image_path = str(self.dataset.samples[index][0])

        return image, label, generator, image_path


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate the gated detector on a manifest split."
    )
    ...
    parser.add_argument(
        "--output",
        default="report/gated_vqdm_predictions.csv",
    )
