"""Train Frequency Stream v1 with stronger robustness augmentation."""
...
from src.core.dataset import (
    ...
    get_frequency_robust_train_transform,
)
...
class FrequencyClassifierV1(nn.Module):
    """Original Frequency v1 architecture."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train Frequency v1 with stronger robustness augmentation."
    )
    ...
    parser.add_argument(
        "--output",
        default="model/frequency/frequency_robust.pt",
    )
    ...
    train_dataset = TwoItemDataset(
        ManifestDataset(
            args.manifest,
            split="train",
            transform=get_frequency_robust_train_transform(),
        )
    )
    ...
    print("augmentation=frequency_robust_v1")
    ...
    for epoch in range(
        ...
        if metrics["roc_auc"] >= best_auc:
            ...
            payload = {
                "format_version": 1,
                "model_name": "frequency_stream_v1_robust",
    ...
    print("Frequency robustness training complete.")
